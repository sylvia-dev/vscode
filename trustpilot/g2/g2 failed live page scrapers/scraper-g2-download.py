...
def parse_html_to_csv(html_file_path, csv_file_path):
    ...
    with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['username', 'title', 'business_type', 'business_size', 'validated_user', 'verified_current_user', 'review_source', 'incentivized_review', 'date', 'rating', 'review_title', 'review_content']
        ...
        for card in review_cards:
            ...
            review_content = card.select_one('div[itemprop="reviewBody"]')
            review_data['review_content'] = ' '.join(p.text.strip() for p in review_content.find_all('p')) if review_content else None
